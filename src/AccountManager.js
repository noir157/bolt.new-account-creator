...
class AccountManager {
  ...
  async extractConfirmationLink(messages) {
    ...
    // Se não encontrou o link nas mensagens de confirmação, tenta uma abordagem mais genérica
    if (!confirmationLink) {
      helpers.log('Tentando abordagem alternativa para extrair link...', 'warn');

      // Tenta em todas as mensagens
      for (const message of messages) {
        try {
          const messageDetails = await this.emailService.getMessageDetails(message.id);

          // Procura por URLs no conteúdo bruto
          ...
          if (allUrls.length > 0) {
            ...
            // Procura por URLs que parecem ser de confirmação
            ...
            // Se não encontrou nenhum que pareça ser de confirmação, usa o primeiro
